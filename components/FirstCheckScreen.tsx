...
const FirstCheckScreen: React.FC<FirstCheckScreenProps> = ({ 
  product, 
  user, 
  onNavigate, 
  showToast 
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/products/${product.barcode}/first-check`, {
        method: 'PATCH',
      ...
      console.error('Error submitting first check:', error);
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-50">
      <header className="bg-white shadow-md p-4 flex items-center sticky top-0 z-10">
        ...
        <div>
          <h1 className="text-xl font-bold text-gray-800">Kiểm tra lần 1</h1>
          ...
          <div className="space-y-2">
            ...
            {/* <label className="flex items-center p-3 border rounded-lg cursor-pointer hover:bg-gray-50 transition">
              <input
                ...
                name="check_result"
                value="incorrect"
                checked={checkData.check_result === 'incorrect'}
                onChange={(e) => setCheckData({ ...checkData, check_result: 'incorrect' })}
                className="mr-3 w-4 h-4"
              />
              <div>
                <div className="font-medium text-red-700">✗ Sai</div>
                <div className="text-xs text-gray-500">Thông tin sai hoàn toàn</div>
              </div>
            </label> */}
          </div>
        </section>
      ...
      <footer className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white p-4 border-t border-gray-200 z-10 shadow-lg">
        <button
          ...
          disabled={loading}
          className="w-full py-3 bg-green-600 text-white rounded-lg font-semibold hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition duration-300 flex justify-center items-center"
        >
          {loading ? (
            <>
